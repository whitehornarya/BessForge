...
export const CRL_SUBSTATION_TRACE_FACTS = Object.freeze({
    source: Object.freeze({
        ...
        tileMode: 1,
        layouts: Object.freeze(['Model', 'Layout1']),
...
function decodeBase64(value) {
    ...
    for (const character of value) {
        ...
        if (digit < 0)
            throw new Error('Invalid CRL substation trace encoding');
    ...
    if (offset !== output.length)
        throw new Error('Truncated CRL substation trace encoding');
    return output;
}
...
function decodeTrace() {
    ...
    for (let pathIndex = 0; pathIndex < pathCount; pathIndex++) {
        if (cursor.offset >= bytes.length)
            throw new Error('Truncated CRL substation trace path');
        const tag = bytes[cursor.offset++];
        const layerIndex = tag & 0x7f;
        const layer = CRL_SUBSTATION_LAYER_KEYS[layerIndex];
        if (!layer)
            throw new Error('Unknown CRL substation trace layer');
        const count = readVarUint(bytes, cursor);
        if (count < 2)
            throw new Error('CRL substation trace path requires at least two points');
        const points = [];
        let x = 0;
        let y = 0;
...
//# sourceMappingURL=crlSubstationTrace.js.map