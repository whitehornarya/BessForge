import {
  CABLE_LAYER, DEFAULT_CALIBRATION_PROFILE, LAYERS, PAGE_HEIGHT_IN, PAGE_MARGIN_IN,
  PAGE_WIDTH_IN, STANDARD_SCALES_FT_PER_IN, TRENCH_LAYER,
} from './constants.js';
import type {
  CableClass, Equipment, LegacyBuildOptions, LegacyCalibrationProfile,
  LegacyLayoutInput, Point, TrenchClass,
} from './types.js';
import {
  CRL_SUBSTATION_PATHS,
...
import {
  normalizeRotationDeg, quantizeDxfNumber, quantizeDxfPoint, quantizedPointKey,
} from './numeric.js';
import {
  DxfWriter,
...
} from './sourceCalibration.js';

const legacyLayerDefinitions: readonly LayerDefinition[] = [
...
type LayerSet = Readonly<Record<LayerKey, string>>;

interface ComposeProfile {
  id: LegacyCalibrationProfile;
  layers: LayerSet;
  ...
  textStyleByLayer: Readonly<Record<string, string>>;
  statusStampTextStyle?: string;
  paperPlanBoxIn?: Readonly<{ left: number; bottom: number; right: number; top: number }>;
  legendColumnIn?: Readonly<{ left: number; right: number }>;
  sourceSubstation?: Readonly<{
    layers: Readonly<Record<CrlSubstationLayerKey, string>>;
    layerDefinitions: readonly LayerDefinition[];
    lineTypes: readonly LineTypeDefinition[];
  }>;
}

const legacyComposeProfile: ComposeProfile = Object.freeze({
...
function resolveComposeProfile(options: LegacyBuildOptions): ComposeProfile {
  ...
  if (profile === DEFAULT_CALIBRATION_PROFILE) return legacyComposeProfile;
...
function canonical<T extends { id: string }>(rows: readonly T[]): T[] {
  return [...rows].sort((a, b) => a.id < b.id ? -1 : a.id > b.id ? 1 : 0);
}

function allPlanPoints(input: LegacyLayoutInput): Point[] {
  const out: Point[] = [...input.siteBoundary.points, ...input.fence.points];
...
function canonicalRing(points: readonly Point[], ccw: boolean): Point[] {
...
function ringSortKey(points: readonly Point[]): string {
...
function drawTrace(
  writer: DxfWriter,
  polygons: ReadonlyArray<ReadonlyArray<ReadonlyArray<ReadonlyArray<number>>>>,
  x: number,
  y: number,
  width: number,
  height: number,
...
function chooseSheet(input: LegacyLayoutInput, options: LegacyBuildOptions): Sheet {
  ...
  const start = STANDARD_SCALES_FT_PER_IN.indexOf(requested as never);
  if (start < 0) throw new Error(`scaleFtPerIn must be one of STANDARD_SCALES_FT_PER_IN; received ${requested}`);
...
function transform(sheet: Sheet, point: Point): Point {
  const source = quantizeDxfPoint(point);
  return quantizeDxfPoint({ x: source.x + sheet.dx, y: source.y + sheet.dy });
}

function rect(writer: DxfWriter, x: number, y: number, w: number, h: number, layer: string): void {
...
function drawBorder(writer: DxfWriter, sheet: Sheet): void {
...
function drawTitle(writer: DxfWriter, input: LegacyLayoutInput, sheet: Sheet): void {
  ...
  for (let i = 1; i <= 3; i++) writer.addLine({ x: x1, y: y + i * rowH }, { x: x2, y: y + i * rowH }, layers.TITLE);
  ...
  engRows.forEach((row, i) => {
    ...
    if (i) writer.addLine({ x: x3, y: y + h - i * h / 4 }, { x: x4, y: y + h - i * h / 4 }, layers.TITLE);
...
function drawNorthAndScale(writer: DxfWriter, sheet: Sheet, panelX: number, panelTop: number): void {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    if (i % 2 === 0) writer.addHatch([box], layers.TITLE, 'SOLID', 7);
    writer.addPolyline(box, layers.TITLE, true);
  }
  [['0', 0], [`${s / 2}FT`, 1], [`${s}FT`, 2], [`${2 * s}FT`, 4]].forEach(([label, index]) =>
    writer.addCenteredText({ x: start + Number(index) * unit, y: barY + 0.28 * s }, 0.1 * s, String(label), layers.TEXT_SM));
  writer.addCenteredText({ x: cx, y: barY - 0.25 * s }, 0.13 * s, `SCALE: 1"=${sheet.scale}'`, layers.TEXT_SM);
}

const legendLabels: readonly [CableClass, string][] = [
...
type LegendGlyph =
  | 'inverter' | 'bess-a' | 'bess-c' | 'aux-distribution' | 'aux-transformer'
  | 'fjb' | 'jb' | 'substation' | 'future' | 'exclusion' | 'line' | 'fence'
  | 'proposed-access' | 'existing-access' | 'detail';

function drawLegendGlyph(
  writer: DxfWriter, glyph: LegendGlyph, x: number, cy: number, w: number, h: number, layer: string,
  useLegacyEntityColors = true,
): void {
  const box = [
    { x, y: cy - h / 2 }, { x: x + w, y: cy - h / 2 },
    { x: x + w, y: cy + h / 2 }, { x, y: cy + h / 2 },
  ];
  const vertical = (fraction: number) => writer.addLine(
    { x: x + fraction * w, y: cy - h / 2 }, { x: x + fraction * w, y: cy + h / 2 }, layer,
  );
  if (glyph === 'line') {
  ...
  if (glyph === 'detail') {
    ...
    writer.addText(
      { x: cx + r * 1.35, y: cy - r * 0.48 },
      r * 0.22,
      'DRAWING ON WHICH DETAIL APPEARS B00X-X',
      layer,
    );
    return;
  }
  if (glyph === 'exclusion') writer.addHatch([box], layer, 'ANSI37');
  if (glyph === 'proposed-access') writer.addHatch([box], layer, 'SOLID', useLegacyEntityColors ? 9 : undefined);
  if (glyph === 'existing-access') writer.addHatch([box], layer, 'SOLID', useLegacyEntityColors ? 8 : undefined);
  ...
  if (glyph === 'inverter') {
    ...
    vertical(0.25); vertical(0.5); vertical(0.75);
    ...
    vertical(0.2); vertical(0.5); vertical(0.8);
    writer.addLine({ x: x + 0.08 * w, y: cy }, { x: x + 0.92 * w, y: cy }, layer);
  }
}

function drawLegend(writer: DxfWriter, input: LegacyLayoutInput, sheet: Sheet): void {
  const s = sheet.scale;
  const layers = sheet.profile.layers;
  ...
  const rows: { label: string; layer: string; glyph: LegendGlyph }[] = [
    ...
    ...legendLabels.map(([klass, label]) => ({ label, layer: sheet.profile.cableLayers[klass], glyph: 'line' as const })),
  ...
  rows.forEach((row, i) => {
    ...
    drawLegendGlyph(writer, row.glyph, sx, cy, sw, 0.26 * s, row.layer,
      sheet.profile.id === DEFAULT_CALIBRATION_PROFILE);
    if (row.label) writer.addText({ x: x + 1.85 * s, y: cy - 0.07 * s }, 0.095 * s, row.label, layers.TEXT_SM);
  });
  ...
  fitText(writer, x + w / 2, stampY + 0.28 * s, 0.26 * s, input.title.statusStamp,
    w - 0.25 * s, layers.TEXT_LG, sheet.profile.statusStampTextStyle);
  ...
  input.title.notes.forEach((note, i) =>
    fitText(writer, x + w / 2, notesY + notesH - (0.55 + i * 0.2) * s, 0.09 * s, `${i + 1}. ${note}`, w - 0.3 * s, layers.TEXT_SM));
  drawNorthAndScale(writer, sheet, x, sheet.height - sheet.margin - 0.2 * s);
}

function rotatedCorners(eq: Equipment, center: Point): Point[] {
...
function drawEquipment(writer: DxfWriter, eq: Equipment, sheet: Sheet): void {
  const layers = sheet.profile.layers;
  ...
  if (sourceSubstation) drawSourceSubstation(writer, eq, center, sourceSubstation.layers);
  else writer.addPolyline(corners, layers.EQUIPMENT, true);
  const rotation = normalizeRotationDeg(eq.rotationDeg);
  const rad = rotation * Math.PI / 180, ux = Math.cos(rad), uy = Math.sin(rad);
  const vx = -uy, vy = ux;
  const line = (along: number, halfWidth: number) => writer.addLine(
    { x: center.x + ux * along - vx * halfWidth, y: center.y + uy * along - vy * halfWidth },
    { x: center.x + ux * along + vx * halfWidth, y: center.y + uy * along + vy * halfWidth },
    layers.EQUIPMENT,
  );
  const side = (across: number, halfLength: number) => writer.addLine(
    { x: center.x + vx * across - ux * halfLength, y: center.y + vy * across - uy * halfLength },
    { x: center.x + vx * across + ux * halfLength, y: center.y + vy * across + uy * halfLength },
    layers.EQUIPMENT,
  );
  if (sourceSubstation) {
    ...
    line(0, eq.width / 2);
    writer.addLine(
      { x: center.x - ux * eq.length * 0.4 - vx * eq.width * 0.3, y: center.y - uy * eq.length * 0.4 - vy * eq.width * 0.3 },
      { x: center.x - ux * eq.length * 0.1 + vx * eq.width * 0.3, y: center.y - uy * eq.length * 0.1 + vy * eq.width * 0.3 },
      layers.EQUIPMENT,
    );
...
export function composeLegacyLayout(input: LegacyLayoutInput, options: LegacyBuildOptions = {}): string {
  ...
  Object.entries(profile.textStyleByLayer).forEach(([layer, style]) =>
    writer.setLayerTextStyle(layer, style));
  drawBorder(writer, sheet);
  drawTitle(writer, input, sheet);
  drawLegend(writer, input, sheet);

  const tx = (points: readonly Point[]) => points.map(p => transform(sheet, p));
  const loops = (polygon: { points: readonly Point[]; holes?: readonly (readonly Point[])[] }) => [
  ...
  canonical(input.surfacing).forEach(p => {
    ...
    writer.addHatch(polygonLoops, layers.SURFACING, 'ANSI31',
      profile.id === DEFAULT_CALIBRATION_PROFILE ? 9 : undefined);
    writer.addPolyline(polygonLoops[0]!, layers.SURFACING, true);
  });
  canonical(input.roads).forEach(road => {
    const roadLoops = loops(road);
    writer.addHatch(roadLoops, layers.ROAD_HATCH, 'SOLID',
      profile.id === DEFAULT_CALIBRATION_PROFILE ? 9 : undefined);
    writer.addPolyline(roadLoops[0]!, layers.ROAD, true);
    if (road.label) {
      const b = bounds(roadLoops[0]!);
      writer.addCenteredText({ x: (b.minX + b.maxX) / 2, y: (b.minY + b.maxY) / 2 },
        0.09 * sheet.scale, road.label, layers.TEXT_SM);
    }
  });
  canonical(input.existingAccess).forEach(road => {
    const roadLoops = loops(road);
    writer.addHatch(roadLoops, layers.ROAD_EXISTING, 'SOLID',
      profile.id === DEFAULT_CALIBRATION_PROFILE ? 8 : undefined);
  ...
  canonical(input.reservedZones).forEach(zone => {
    ...
    writer.addCenteredText({ x: (b.minX + b.maxX) / 2, y: (b.minY + b.maxY) / 2 },
      0.09 * sheet.scale, zone.label, layers.TEXT_SM);
  });
  ...
  canonical(input.cables).forEach(run => {
    ...
    if (run.feederName) writer.addCenteredText(pathMidpoint(points), 0.08 * sheet.scale,
      run.feederName, layers.TEXT_SM);
  });
  ...
  canonical(input.matchLines).forEach(line => {
    ...
    writer.addCenteredText({ x: (first.x + last.x) / 2 + 0.2 * sheet.scale, y: (first.y + last.y) / 2 },
      0.16 * sheet.scale, line.label, layers.TEXT_MD, 90);
  });
  canonical(input.callouts).forEach(callout => {
    ...
    writer.addText({ x: textPoint.x + 0.05 * sheet.scale, y: textPoint.y + 0.025 * sheet.scale },
      0.075 * sheet.scale, callout.text, layers.TEXT_SM);
  });
  canonical(input.detailMarkers).forEach(marker => {
    ...
    fitText(writer, center.x, center.y + r * 0.28, 0.085 * sheet.scale,
      marker.detailId, 1.55 * r, layers.TEXT_MD);
    fitText(writer, center.x, center.y - r * 0.58, 0.065 * sheet.scale,
      marker.drawingId, 1.55 * r, layers.TEXT_SM);
  });
  return writer.toString();
}

export function resolveLegacyScale(input: LegacyLayoutInput, options: LegacyBuildOptions = {}): number {
