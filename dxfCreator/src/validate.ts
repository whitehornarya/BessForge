import {
  CABLE_CLASSES, EQUIPMENT_KINDS, STANDARD_SCALES_FT_PER_IN, TRENCH_CLASSES,
} from './constants.js';
import type {
  LegacyLayoutInput, Point, ValidationIssue, ValidationResult,
} from './types.js';
import {
  MAX_ABS_LOCAL_COORDINATE_FT, MAX_ABS_ROTATION_DEG, MAX_EQUIPMENT_DIMENSION_FT,
  MIN_EQUIPMENT_DIMENSION_FT, quantizeDxfPoint, quantizedPointKey,
} from './numeric.js';
...
function pointInRing(point: Point, ring: readonly Point[]): boolean {
  if (ring.some((p, i) => pointOnSegment(point, p, ring[(i + 1) % ring.length]!))) return false;
  let inside = false;
...
function quantizedRing(value: unknown): Point[] | undefined {
...
export function assertValidLegacyLayoutInput(input: unknown): LegacyLayoutInput {
  const result = validateLegacyLayoutInput(input);
  if (!result.ok) throw new LegacyLayoutValidationError(result.issues);
  return result.value;
}

export function validateLegacyLayoutInput(input: unknown): ValidationResult {
  const issues: ValidationIssue[] = [];
  const issue = (path: string, code: string, message: string) => issues.push({ path, code, message });
  const record = (value: unknown, path: string): Record<string, unknown> | undefined => {
    if (!isRecord(value)) {
  ...
  const text = (value: unknown, path: string, allowEmpty = false, maxLength = 128): void => {
  ...
  const segment = (a: unknown, b: unknown, path: string): void => {
    if (!isRecord(a) || !isRecord(b) ||
        !validCoordinate(a.x) || !validCoordinate(a.y) ||
        !validCoordinate(b.x) || !validCoordinate(b.y)) return;
    if (quantizedPointKey(quantizeDxfPoint(a as unknown as Point)) ===
        quantizedPointKey(quantizeDxfPoint(b as unknown as Point))) {
      issue(path, 'geometry.quantizedZeroLengthSegment',
        'segment endpoints collapse together at six-decimal DXF precision');
    }
  };
  const points = (value: unknown, path: string, min: number, polygon = false): void => {
    const rows = array(value, path);
    if (rows.length < min) issue(path, 'geometry.vertices', `must contain at least ${min} points`);
    rows.forEach((p, i) => point(p, `${path}[${i}]`));
    const sourceValid = rows.every(p =>
      isRecord(p) && validCoordinate(p.x) && validCoordinate(p.y));
    if (sourceValid) {
      const raw = rows as unknown as Point[];
      ...
      for (let i = 0; i + 1 < pts.length; i++) {
        if (quantizedPointKey(pts[i]!) === quantizedPointKey(pts[i + 1]!)) {
          ...
          issue(`${path}[${i}]`, exact ? 'geometry.zeroLengthSegment' : 'geometry.quantizedZeroLengthSegment',
            exact ? 'must not duplicate the next point' :
      ...
      if (polygon && pts.length >= 3 &&
        ...
        issue(`${path}[${pts.length - 1}]`, 'geometry.quantizedZeroLengthClosingEdge',
          'closing edge collapses at six-decimal DXF precision');
      }
    }
    if (polygon && rows.length >= 3 && sourceValid) {
      const pts = (rows as unknown as Point[]).map(quantizeDxfPoint);
      ...
      if (unique.size < 3) issue(path, 'geometry.distinct', 'polygon must have at least three distinct points');
  ...
  const identifiedPolyline = (value: unknown, path: string, polygon = false): void => {
  ...
  if (title) {
    ...
    if (notes.length < 1 || notes.length > 3) issue('$.title.notes', 'array.cardinality', 'must contain 1..3 notes');
    notes.forEach((v, i) => text(v, `$.title.notes[${i}]`, false, 72));
    const revisions = array(title.revisions, '$.title.revisions');
    if (revisions.length < 1 || revisions.length > 3) issue('$.title.revisions', 'array.cardinality', 'must contain 1..3 revision rows');
    ...
    revisions.forEach((v, i) => {
      ...
      if (row) {
        ...
        if (typeof row.revision === 'string') {
          if (revisionKeys.has(row.revision)) issue(`$.title.revisions[${i}].revision`, 'revision.duplicate', 'must be unique');
  ...
  if (scale) {
    ...
    if (typeof scale.preferredFtPerIn === 'number' &&
        !(STANDARD_SCALES_FT_PER_IN as readonly number[]).includes(scale.preferredFtPerIn)) {
      issue('$.scale.preferredFtPerIn', 'scale.standard', 'must be one of STANDARD_SCALES_FT_PER_IN');
    }
    if (typeof scale.autoScale !== 'boolean') issue('$.scale.autoScale', 'type.boolean', 'must be a boolean');
  }
  ...
  const registerId = (value: unknown, path: string) => {
    ...
    if (prior) issue(`${path}.id`, 'id.duplicate', `duplicates ${prior}`);
    else seen.set(value.id, `${path}.id`);
  };
  ...
  const collections = ['adjacentFences', 'projectFences', 'existingAccess', 'detailMarkers',
    'roads', 'surfacing', 'equipment', 'cables', 'trenches', 'reservedZones', 'callouts', 'matchLines'] as const;
  const rows = Object.fromEntries(collections.map(k => [k, array(root[k], `$.${k}`)])) as Record<typeof collections[number], unknown[]>;
  ...
  rows.existingAccess.forEach((v, i) => {
    ...
    if (row?.label !== undefined) text(row.label, `$.existingAccess[${i}].label`, true, 64);
  });
  ...
  rows.roads.forEach((v, i) => {
    ...
    const row = isRecord(v) ? v : undefined;
    if (row?.label !== undefined) text(row.label, `$.roads[${i}].label`, true, 48);
  });
  ...
  rows.equipment.forEach((v, i) => {
    ...
    if (typeof row.kind !== 'string' || !(EQUIPMENT_KINDS as readonly string[]).includes(row.kind)) {
    ...
    for (const key of ['length', 'width'] as const) {
      ...
      if (typeof value === 'number' && Number.isFinite(value) &&
        ...
        issue(`${path}.${key}`, 'number.equipmentDimensionRange',
          `must be ${MIN_EQUIPMENT_DIMENSION_FT}..${MAX_EQUIPMENT_DIMENSION_FT} ft`);
      }
    }
    if (typeof row.rotationDeg === 'number' && Number.isFinite(row.rotationDeg) &&
        Math.abs(row.rotationDeg) > MAX_ABS_ROTATION_DEG) {
      issue(`${path}.rotationDeg`, 'number.rotationRange',
        `absolute raw rotation must be <= ${MAX_ABS_ROTATION_DEG} degrees`);
    }
    if (row.kind === 'bess') {
  ...
  rows.cables.forEach((v, i) => {
    ...
    const row = isRecord(v) ? v : undefined;
    if (typeof row?.class !== 'string' || !(CABLE_CLASSES as readonly string[]).includes(row.class)) {
      issue(`${path}.class`, 'enum.cableClass', `must be one of ${CABLE_CLASSES.join(', ')}`);
    }
    if (row?.feederName !== undefined) text(row.feederName, `${path}.feederName`, false, 24);
  });
  rows.trenches.forEach((v, i) => {
    const path = `$.trenches[${i}]`;
    identifiedPolyline(v, path);
    const row = isRecord(v) ? v : undefined;
    if (typeof row?.class !== 'string' || !(TRENCH_CLASSES as readonly string[]).includes(row.class)) {
  ...
  return issues.length
    ? { ok: false, issues }
    : { ok: true, value: input as LegacyLayoutInput };
}