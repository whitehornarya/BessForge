...
export class DxfWriter {
  ...
  private readonly lineTypes: string[] = [];
  private readonly textStyles: string[] = [];
  private readonly layerTextStyles = new Map<string, string>();
  private handle = 0x100;

  constructor(private readonly lineTypeScale = 1) {
  ...
  private entity(type: string, layer: string): string {
    return `  0\n${type}\n  5\n${this.nextHandle()}\n330\n${MODEL_SPACE_RECORD}\n100\nAcDbEntity\n  8\n${layer}`;
  }
  ...
  addPolyline(points: readonly Point[], layer: string, closed = false, color?: number): void {
    if (points.length < 2) throw new Error('LWPOLYLINE requires at least two points');
    let s = this.entity('LWPOLYLINE', layer);
    if (color !== undefined) s += `\n 62\n${color}`;
    s += `\n100\nAcDbPolyline\n 90\n${points.length}\n 70\n${closed ? 1 : 0}`;
    for (const point of points) s += `\n 10\n${formatDxfNumber(point.x)}\n 20\n${formatDxfNumber(point.y)}`;
    this.entities.push(s);
  }
  ...
  addText(point: Point, height: number, text: string, layer: string, rotationDeg = 0, style?: string): void {
    ...
    if (textStyle && textStyle !== 'Standard') s += `\n  7\n${textStyle}`;
    s += '\n100\nAcDbText';
    this.entities.push(s);
  }

  addCenteredText(point: Point, height: number, text: string, layer: string, rotationDeg = 0, style?: string): void {
    const rotation = normalizeRotationDeg(rotationDeg);
    let s = `${this.entity('TEXT', layer)}\n100\nAcDbText\n 10\n${formatDxfNumber(point.x)}\n 20\n${formatDxfNumber(point.y)}\n 30\n0\n 40\n${formatDxfNumber(height)}\n  1\n${esc(text)}`;
    if (rotation) s += `\n 50\n${formatDxfNumber(rotation)}`;
    const textStyle = style ?? this.layerTextStyles.get(layer);
  ...
  toString(): string {
    ...
    for (const [handle, name, description, elements] of ltypes) {
      const total = elements.reduce<number>((sum, value) => sum + Math.abs(value), 0);
      dxf += `  0\nLTYPE\n  5\n${handle}\n330\n5\n100\nAcDbSymbolTableRecord\n100\nAcDbLinetypeTableRecord\n  2\n${name}\n 70\n0\n  3\n${description}\n 72\n65\n 73\n${elements.length}\n 40\n${formatDxfNumber(total)}\n`;
      elements.forEach(value => { dxf += ` 49\n${formatDxfNumber(value)}\n 74\n0\n`; });
    }
    if (this.lineTypes.length) dxf += this.lineTypes.join('');
    dxf += '  0\nENDTAB\n';
